...
let RawDataComponent = class RawDataComponent {
    ...
    __populate() {
        ...
        for (const name in this.functions) {
            if (this.functions.hasOwnProperty(name) && !(name.toLowerCase() === 'date')) {
                const functions = this.functions[name];
                let dataline = d3__WEBPACK_IMPORTED_MODULE_2__["line"]()
                    // d3 namespace
...
let TestComponent = class TestComponent {
    ...
    ngOnInit() {
        ...
        this.functions = {};
        this.keys = [];
        this.api.get('raw_data').subscribe((raw_data) => {
...
let ApiService = class ApiService {
    ...
    post(resource, body) {
        ...
        let url = `${src_environments_environment__WEBPACK_IMPORTED_MODULE_4__["environment"].api}/${resource}`;
        let response = this.http.post(url, body, {
            withCredentials: true,
        }).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["map"])(x => {
            if (!(src_environments_environment__WEBPACK_IMPORTED_MODULE_4__["environment"].production)) {
                console.log(this.constructor.name, __function__, 'url', url, x);
            }
            return x;
        }));
        return response;
    }
    get(resource, id = null, params = {}) {
    ...
    put(resource, id, body) {
        ...
        let url = `${src_environments_environment__WEBPACK_IMPORTED_MODULE_4__["environment"].api}/${resource}/${id}`;
    ...
    delete(resource, id = null, params = {}) {
